...
PABLO = "data/calidad_scoring_pablo.csv"
MIGUEL = "data/calidad_scoring_miguel.csv"
...
def main():
    key = load_key()
    miguel = load_scores(MIGUEL, key)
    pablo  = load_scores(PABLO, key)
    blank  = load_scores(BLANK, key)

    # For miguel csv, detect error rows: qwen3-vl with no valid scores = error
    all_keys = set(miguel.keys()) | set(pablo.keys()) | set(blank.keys())

    # Build per-model, per-group data
    # merge miguel+pablo (two annotators) into consensus scores
    ...
    # determine group for each id from miguel or blank
    id_groups = {}
    for path in [MIGUEL, PABLO, BLANK]:
        for r in csv.DictReader(open(path, encoding="utf-8")):
            id_groups[r["id"]] = r["grupo"]

    # For each (id, model) in miguel, check if valid output
    for (iid, model), scores in miguel.items():
